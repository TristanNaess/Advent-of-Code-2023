...
class Sequence
{
    public:
        Sequence(const std::string& line)
        {
            ...
            vals.push_back(std::vector<long int>{});
        ...
        void calc_next_val()
        {
            ...
            while (!all_zeros)
            {
                ...
                differences.reserve(prev.size() - 1);
    ...
    private:
        std::vector<std::vector<long int>> vals;
...
int main(int argc, char** argv)
{
    ...
    long int sum = std::ranges::fold_left(sequences.cbegin(), sequences.cend(), 0, [](long int sum, const Sequence& s){ sum += s.last_val(); return sum; });

    std::cout << "Sum of extrapolated vals: " << sum << '\n';
}
